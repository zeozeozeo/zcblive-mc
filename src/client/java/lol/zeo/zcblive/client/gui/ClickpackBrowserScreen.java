...
public final class ClickpackBrowserScreen extends Screen {
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
		...
		if (sortDropdownOpen && sortDropdownButton != null) {
			...
			graphics.outline(dropdownX, dropdownY, dropdownWidth, dropdownHeight, 0xFF4A4A4A);
		}
		graphics.text(font, statusText, 12, height - 72, 0xFFD0D0D0, false);
	...
	private void renderDetailPanel(GuiGraphicsExtractor graphics, int x, int y, int width, int height) {
		...
		drawY += 10;
		graphics.text(font, Component.literal("Downloads: " + formatDownloads(selectedEntry.downloadCount())), x + 8, drawY, 0xFFD0D0D0, false);
		drawY += 10;
	...
	private final class ClickpackEntry extends ObjectSelectionList.Entry<ClickpackEntry> {
		...
		public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float a) {
			...
			graphics.text(font, Component.literal(downloads), downloadsX, getContentY(), 0xFF8F8F8F, false);
